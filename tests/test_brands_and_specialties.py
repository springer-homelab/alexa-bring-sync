...
def test_brand_generic_catalog_decomposition(nlu, catalog):
    # Gustavo Gusto Pizza -> Pizza (Gustavo Gusto)
    ...
    # Gerolsteiner Mineralwasser -> Mineralwasser (Gerolsteiner)
    p3 = nlu.parse_items("Gerolsteiner Mineralwasser", catalog)
    assert p3[0]["name"] == "Mineralwasser"
    assert "Gerolsteiner" in p3[0]["specification"]

    # Kerrygold Butter -> Butter (Kerrygold)
    p4 = nlu.parse_items("Kerrygold Butter", catalog)
    assert p4[0]["name"] == "Butter"
    assert "Kerrygold" in p4[0]["specification"]

    # Barilla Spaghetti -> Spaghetti (Barilla)
    p5 = nlu.parse_items("Barilla Spaghetti", catalog)
    assert p5[0]["name"] == "Spaghetti"
    assert "Barilla" in p5[0]["specification"]
...
def test_grain_styles_and_diets(nlu, catalog):
    # Vollkorntoast -> Toast (Vollkorn)
    p1 = nlu.parse_items("Vollkorntoast", catalog)
    assert p1[0]["name"] == "Toast"
    assert "Vollkorn" in p1[0]["specification"]

    # Toast Vollkorn -> Toast (Vollkorn)
    p2 = nlu.parse_items("Toast Vollkorn", catalog)
    assert p2[0]["name"] == "Toast"
    assert "Vollkorn" in p2[0]["specification"]

    # Dinkelbrot -> Brot (Dinkel)
    p3 = nlu.parse_items("Dinkelbrot", catalog)
    assert p3[0]["name"] == "Brot"
    assert "Dinkel" in p3[0]["specification"]

    # Glutenfreie Nudeln -> Nudeln (Glutenfrei)
    p4 = nlu.parse_items("glutenfreie Nudeln", catalog)
    assert p4[0]["name"] == "Nudeln"
    assert "Glutenfrei" in p4[0]["specification"]
